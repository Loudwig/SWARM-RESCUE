...
class MyDroneSemantic(DroneAbstract):
    ...
    def control_random(self):
        ...
        if collided and not self.isTurning and self.counterStraight > 10:
            self.isTurning = True
            ...
            if lidar_values is not None:
                ...
                #    if distance > 40:  # Ensure it's an open direction
                #        # Calculate the target position based on this direction
                #        target_x = x + math.cos(i) * self.lencell
                #        target_y = y + math.sin(i) * self.lencell
                #        grid_x = int(target_x // self.lencell)
                #        grid_y = int(target_y // self.lencell)
                #        if not self.visited[(grid_x, grid_y)]:
                #            unexplored_angles.append((i, distance))

                #if unexplored_angles:
                    # Choose the angle leading to the farthest unexplored area

                #    self.angleStopTurning = max(unexplored_angles, key=lambda x: x[1])[0]
                #else:
                self.angleStopTurning = np.argmax(lidar_values)
            else:
        ...
        if self.isTurning:
            kp = 1.5  # vitesse rot
            rotation_speed = kp * diff_angle
            rotation_speed = max(-1.0, min(1.0, rotation_speed)) # clip la rotation
            command_turn["rotation"] = rotation_speed
