...
class MyDronePID_angle(DroneAbstract):
    class State(Enum):
        """
...
###########################PATH CHOICE##########################
    ...
    def log_pid_values(self):
        ...
        csv_file = os.path.join(log_dir, f"{self.id_log_file}_pid_values_angle.csv")
        
        # Create file with headers if it doesn't exist
        if not os.path.exists(csv_file):
            with open(csv_file, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['timestep', 'epsilon_angle'])
        
        # Append values
        with open(csv_file, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([self.timestep_count, self.epsilon_angle])
