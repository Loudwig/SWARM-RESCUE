...
from map_editor.walls_boxes2 import add_walls, add_boxes, dimensions


class MyMapboxes2(MapAbstract):
    ...
    def __init__(self, zones_config: ZonesConfig = ()):
        ...
        self._rescue_center = RescueCenter(size=(260, 129))
        self._rescue_center_pos = ((-265, 233), 0)

        self._return_area = ReturnArea(size=(268, 135))
        self._return_area_pos = ((-260, 229), 0)

        self._wounded_persons_pos = []
        self._number_wounded_persons = len(self._wounded_persons_pos)
        self._wounded_persons: List[WoundedPerson] = []

        self._drones_pos = [(pos, random.uniform(-math.pi, math.pi)) for pos in [(-130, 168)]]
