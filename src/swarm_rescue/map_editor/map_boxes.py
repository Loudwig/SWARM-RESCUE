...
from map_editor.walls_boxes import add_walls, add_boxes, dimensions


class MyMapboxes(MapAbstract):
    ...
    def __init__(self, zones_config: ZonesConfig = ()):
        ...
        self._rescue_center = RescueCenter(size=(343, 139))
        self._rescue_center_pos = ((-564, 405), 0)

        self._return_area = ReturnArea(size=(540, 261))
        self._return_area_pos = ((-508, 361), 0)

        self._wounded_persons_pos = []
        self._number_wounded_persons = len(self._wounded_persons_pos)
        self._wounded_persons: List[WoundedPerson] = []

        self._drones_pos = [(pos, random.uniform(-math.pi, math.pi)) for pos in [(-517, 266)]]
