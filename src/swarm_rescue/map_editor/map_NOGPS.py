...
from map_editor.walls_NOGPS import add_walls, add_boxes, dimensions


class MyMapNOGPS(MapAbstract):
    ...
    def __init__(self, zones_config: ZonesConfig = ()):
        ...
        self._no_gps_zone = NoGpsZone(size=(588, 465))
        self._no_gps_zone_pos = ((61, -6), 0)

        self._rescue_center = RescueCenter(size=(101, 47))
        self._rescue_center_pos = ((-347, 273), 0)

        self._return_area = ReturnArea(size=(130, 69))
        self._return_area_pos = ((-335, 266), 0)

        self._wounded_persons_pos = [(-210, 39), (-81, -18)]
        self._number_wounded_persons = len(self._wounded_persons_pos)
        self._wounded_persons: List[WoundedPerson] = []

        self._drones_pos = [(pos, random.uniform(-math.pi, math.pi)) for pos in [(-312, 236)]]
