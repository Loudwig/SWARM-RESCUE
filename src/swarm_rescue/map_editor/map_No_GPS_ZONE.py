...
from map_editor.walls_No_GPS_ZONE import add_walls, add_boxes, dimensions


class MyMapNo_GPS_ZONE(MapAbstract):

    def __init__(self, zones_config: ZonesConfig = ()):
        super().__init__(zones_config)
        self._max_timestep_limit = 5000
        self._max_walltime_limit = 520

        # PARAMETERS MAP
        self._size_area = dimensions()

        self._no_gps_zone = NoGpsZone(size=(321, 292))
        self._no_gps_zone_pos = ((121, 34), 0)

        self._rescue_center = RescueCenter(size=(79, 47))
        self._rescue_center_pos = ((-360, 277), 0)

        self._return_area = ReturnArea(size=(81, 36))
        self._return_area_pos = ((-360, 233), 0)

        self._wounded_persons_pos = [(257, 155), (264, -93), (129, 45)]
        self._number_wounded_persons = len(self._wounded_persons_pos)
        self._wounded_persons: List[WoundedPerson] = []

        self._drones_pos = [(pos, random.uniform(-math.pi, math.pi)) for pos in [(-341, 231)]]
